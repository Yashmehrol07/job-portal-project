...
export default function EmployerDashboard() {
    ...
    const [employerApplications, setEmployerApplications] = useState<Array<{ applicationId: string, status: string, coverLetter: string | null, appliedAt: string, applicantId: string, applicantFirstName: string, applicantLastName: string, applicantEmail: string, jobId: string, jobTitle: string, jobCompany: string }>>([]);
    const [isLoadingEmployerApps, setIsLoadingEmployerApps] = useState(false);
    ...
    useEffect(() => {
        ...
        const fetchEmployerApplications = async () => {
            ...
            try {
                ...
                setIsLoadingEmployerApps(false);
            }
        };
                ...
                <main className={styles.mainContent}>
                    <div className={styles.header}>
                        <h1 className={styles.pageTitle}>
                            {activeTab === 'overview' && 'Employer Overview'}
                            {activeTab === 'post' && (editingJobId ? 'Edit Job Posting' : 'Create New Job Posting')}
                            {activeTab === 'manage' && 'Manage Your Jobs'}
                            ...
                            {isLoadingEmployerApps ? (
                                ...
                                <div className={styles.jobsList}>
                                    {employerApplications.map((app) => (
                                        <div key={app.applicationId} style={{ padding: '1rem', borderBottom: '1px solid var(--border)', backgroundColor: 'var(--surface)', borderRadius: 'var(--radius-md)', marginBottom: '1rem' }}>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                                                    <select
                                                        ...
                                                        onChange={async (e) => {
                                                            ...
                                                            try {
                                                                const res = await fetch(`/api/employer/applications/${app.applicationId}`, {
                                                                    method: 'PUT',
                                                                    headers: { 'Content-Type': 'application/json' },
                                                                    body: JSON.stringify({ status: newStatus })
                                                                });
                                                                if (res.ok) {
                                                                    setEmployerApplications(prev => prev.map(a => a.applicationId === app.applicationId ? { ...a, status: newStatus } : a));
                                                                }
                                                            } catch (err) {
